	...
	if ($product->get_type() == 'crowdfunding') {
		if (wpcf_function()->is_campaign_valid()) {
            ...
            <span class="wpneo-tooltip">
                <span class="wpneo-tooltip-min"><?php echo esc_html__('Minimum amount is ','krowd'); echo esc_html($currency . $min_price); ?></span>
                <span class="wpneo-tooltip-max"><?php echo esc_html__('Maximum amount is ','krowd'); echo esc_html($currency . $max_price); ?></span>
            </span>
			...
			<?php $extra_link = get_post_meta( $post->ID, 'krowd_project_link', true ); ?>
	            ...
	            <form enctype="multipart/form-data" method="post" class="cart_form AA">
	                ...
	                <button type="submit" class="btn-back-campaign btn-theme-2 <?php echo apply_filters('add_to_donate_button_class', 'wpneo_donate_button'); ?>"><span><?php echo esc_html__('Back Campaign', 'krowd'); ?></span></button>
	            </form>

	         <?php }else{ ?>
	         	<a href="<?php echo esc_url($extra_link) ?>" class="btn-back-campaign btn-theme-2"><span><?php echo esc_html__('Back Campaign', 'krowd'); ?></span></a>
	         <?php } ?>
		...
		<?php
			...
			if ( ! wpcf_function()->is_campaign_started()){
				...
				if( wpcf_function()->is_reach_target_goal() ){
					echo esc_html__('The campaign is successful.','krowd');
				}else{
					echo esc_html__('This campaign has been invalid or not started yet.','krowd');
				}
			}
		}
	}
